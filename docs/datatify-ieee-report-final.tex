...
Existing work addresses music behavioral analysis, graph-theoretic listening
models, listener clustering, and AI-generated narrative as independent
research threads. No published open platform combines all five analytical
layers—behavioral metrics, personality profiling, artist-transition graph,
cross-user clustering, and AI narrative—in a single end-to-end system
operating directly on standard platform export data. Datatify fills this gap:
it requires neither Spotify API credentials nor audio features, processes raw
event logs in a single deployable pipeline, and provides a live public
benchmark infrastructure for comparative listener profiling. The three-tier
architecture additionally enables direct empirical comparison of pure-Python,
Pandas, and PySpark processing strategies under controlled conditions—a
contribution absent from prior personalized music analytics platforms.
...
list, count, and average session duration in hours.
The 30-minute boundary is consistent with session-segmentation conventions
adopted in music streaming research~\cite{brost2019sessions}. The constant
$G$ is centralized in \texttt{constants.SESSION\_GAP\_MINUTES} so that all
...
\textbf{Artist Diversity Entropy}~\cite{shannon1948mathematical} is the Shannon entropy of the artist
...
Threshold values were placed at the empirical 75th-percentile boundary of
each relevant metric across the benchmark pool, ensuring that each
specialized archetype captures the most pronounced behavioral patterns
before the default fallback applies.
...
(Fig.~\ref{fig:silhouette}).
The range $k \in [2, 8]$ was determined empirically: silhouette scores
plateau beyond $k = 6$ on benchmark pools exceeding 100 submissions
(additional clusters fragment rather than separate behavior), and
$k_{\min} = 2$ prevents degenerate single-group results.
...
\end{axis}
\end{tikzpicture}
\caption{Silhouette score vs.\ $k$ for K-Means++ on a representative
benchmark pool of 200 submissions. Score peaks at $k^*=4$; the plateau
and decline beyond $k=5$ confirm that $k_{\max}=8$ is a safe upper bound.}
...
Datatify is packaged as a Docker container to achieve environment
reproducibility across development and cloud environments.
The \texttt{Dockerfile} uses the official \texttt{python:3.11-slim} base
image, keeping the final image footprint minimal:
...
A \texttt{.dockerignore} file excludes benchmarking artefacts, test
suites, LaTeX sources, and development-only directories, reducing the
build context transferred to the cloud builder. The \texttt{PORT}
environment variable is read at startup, making the container
compatible with any cloud platform that injects a dynamic port.

\subsubsection{AWS Elastic Beanstalk Deployment}

The primary production deployment targets \textbf{Amazon Web Services
(AWS) Elastic Beanstalk}~\cite{aws_eb}, a Platform-as-a-Service (PaaS)
layer that automates infrastructure provisioning, load balancing, and
health monitoring over standard EC2 compute. Elastic Beanstalk was
selected over raw EC2 because it provides zero-configuration
auto-healing (automatic instance replacement on health check failure),
rolling deployment support, and a managed deployment pipeline
integrated with the AWS ecosystem.

The deployment uses the \textbf{Docker platform on Amazon Linux 2}
(\texttt{64bit Amazon Linux 2 / 4.9.0}) running on a
\texttt{t3.micro} instance (2\,vCPU, 1\,GB RAM) in the
\texttt{eu-west-1} (Ireland) region, chosen for geographic proximity
to the EU user base. Table~\ref{tab:infra} summarises the
...
Deployment is managed via the \textbf{AWS EB CLI}. The workflow is
git-driven: the CLI calls \texttt{git archive} to create a ZIP bundle
of the committed working tree, uploads it to an auto-provisioned S3
bucket, and instructs the EC2 instance to pull and rebuild the Docker
image. Uncommitted changes are intentionally excluded, enforcing the
discipline that every deployed version corresponds to a recorded git
commit.

\begin{enumerate}
  \item \textbf{Initialise} (once): \texttt{eb init -p docker datatify
        --region eu-west-1} registers the application in the EB control
        plane and writes a local \texttt{.elasticbeanstalk/config.yml}.
  \item \textbf{Provision} (once): \texttt{eb create datatify-env
        --single --instance-type t3.micro} allocates the EC2 instance,
        security group, Elastic IP, and S3 bucket; performs the first
        Docker build on the instance; and registers a health-check
        endpoint.
  \item \textbf{Redeploy} (on each change): \texttt{git commit} followed
        by \texttt{eb deploy} uploads the new archive, triggers a
        rolling in-place Docker rebuild, and restores the health-check
        green status.
  \item \textbf{Configuration}: secrets are injected at runtime via
        \texttt{eb setenv GEMINI\_API\_KEY=\ldots\ DB\_PATH=\ldots},
        which writes key-value pairs to the instance environment
        without ever touching the source repository.
\end{enumerate}

The entire provisioning sequence—from a fresh AWS account to a publicly
reachable HTTPS endpoint—requires four CLI commands and completes in
under ten minutes.

\subsubsection{Environment Configuration}

All runtime parameters are supplied exclusively through environment
variables (Table~\ref{tab:env}), ensuring the Docker image is fully
portable and contains no embedded secrets.
...
Elastic Beanstalk satisfies the course infrastructure requirements on
three dimensions: \textit{scalability}—the environment supports
vertical scaling (instance type upgrade) and horizontal scaling
(load-balanced multi-instance tier) with a single configuration
change, enabling the system to handle the 500\,K–2\,M record benchmark
workloads used in Section~\ref{sec:benchmark}; \textit{storage}—the
S3 deployment bucket provides durable, versioned artifact storage for
every deployed bundle, while the SQLite benchmark database persists on
the EC2 EBS volume; and \textit{collaboration}—the live public URL
allows team members and evaluators to submit their own Spotify data
...
The database is gitignored and persists on the EC2 EBS volume. Two API
